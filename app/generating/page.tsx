...
export default function GeneratingPage() {
  ...
  // Slowly animate progress forward even while waiting
  // so it doesn't look stuck
  useEffect(() => {
    const t = setInterval(() => {
      setProgress((p) => {
        if (p >= 90) return p; // stop auto-advance near the end
        return p + 0.3;
      });
    }, 1000);
    return () => clearInterval(t);
  ...
  async function startGeneration() {
    try {
      setProgress(5);
      setStatusText("Uploading drawing...");
      ...
      setProgress(15);
      setStatusText("AI is working on your character...");
  ...
  async function pollStatus(taskId: string) {
    try {
      ...
      if (!res.ok) return; // silently retry on network errors
      ...
      if (data.status === "success") {
        if (pollingRef.current) clearInterval(pollingRef.current);
        setProgress(100);
        setStatusText("Done! Opening your character...");
        update({
          glbUrl: data.glbUrl,
      ...
      if (data.status === "failed") {
        if (pollingRef.current) clearInterval(pollingRef.current);
        setError("The AI couldn't process this drawing. Please try again with a clearer image.");
        return;
      }

      // Still running — use Tripo's real progress if available
      if (data.progress && data.progress > 15) {
        const mapped = 15 + (data.progress / 100) * 75;
        setProgress((prev) => Math.max(prev, mapped));
      }

      // Update status text based on progress
      setProgress((p) => {
        if (p < 30) setStatusText("Analyzing your drawing...");
      ...
      // Don't show error — just retry next interval
    }
  }
  ...
  return (
    <section className="mx-auto max-w-2xl px-6 lg:px-10 py-24 lg:py-32 flex flex-col items-center text-center">
      ...
      <p className="mt-8 text-sm text-navy/40 max-w-xs">
        Our AI is crafting something one-of-a-kind from the drawing.
        This takes about 60–120 seconds — worth the wait! ✨
      </p>
    </section>
  );
}
